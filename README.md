...
Documents include all fields covered by MT's search, with HTML converted to text. Content is not truncated, summarized, or split into chunks. Image alt text is included, but file contents, linked pages, the body of referenced objects, and fields outside MT's search scope are not read.
...
- OpenAI embedding generation accepts up to 8192 tokens per input. Inputs that exceed this limit cause an API error and stop processing.
